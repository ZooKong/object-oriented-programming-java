...
이를 위해서는 매표소가 있으며 매표소에는 판매원이 있다.
...
개선 후에는 Theater가 모든 책임을 갖고 있지 않다.
티켓을 판매하는 책임은 ticketSeller에게 있고 티켓을 구매하는 행위는 audience에게 책임이 있다.

#### 1.4.3. 트레이드 오프

TicketOffice에게도 자율성을 추가한다.
...
TicketOffice 클래스에 sellTicketTo 메서드를 추가하였다.
sellTicketTo 메서드는 관람객을 받아 직접 관람객이 티켓을 구매하도록 하고 돈을 보관하는 능동적인 메서드이다.

그러나 TicketOffice 클래스에 Audience 의존성이 추가되었다.
TicketOffice가 Audience에게 직접 티켓을 판매하고 있고 Audience 클래스를 알아야 한다.
결론적으로 자율성을 향상했더니 결합도가 상승했다.
여기서 자율성과 결합도 상승에 대해 선택을 해야한다.
트레이드 오프를 해야할 수 있다.

#### 1.4.4. 의인화

관람객인 Audience 클래스와 판매원인 TicketSeller 클래스가 능동적으로 행동하는 것은 일반적인 직관과 일치한다.
그러나 Theater, TicketOffice 클래스가 자율성을 갖는 것은 직관적으로 이해하기 힘들다.
왜냐하면 Theater, TicketOffice는 생명이 없는 자율적인 존재가 아니라는 직관이 있기 때문이다.
그러나 객체지향 세계에서는 수동적인 존재라도 능동적이고 자율적인 존재로 바뀐다.
즉, 의인화를 한다.

#### 1.4.5. 객체지향 설계

코드는 오늘 완성해야 하는 기능이면서 내일 쉽게 변경할 수 있는 코드가 좋은 코드다.
또한 요구사항은 항상 변경되기 때문에 변경을 수용할 수 있는 설계가 중요하다.
그러나 모든 요구사항을 초기에 수집할 수 없고 중간에 수정될 가능성이 매우 높다.
그렇기에 변경에 유연하게 대응할 수 있는 코드를 작성해야 한다.

##### 1.4.5.1 코드 수정을 피하도록 설계

객체지향적 설계를 통해 의존성을 낮추고 응집도를 높게 설계한다면 요구사항 변경으로 인한 코드 수정이 적어진다.
코드를 변경한다는 것은 버그를 추구할 가능성을 내포하고 있다.
그렇기에 코드 수정을 적은 방향으로 설계가 되는 것이 바람직하다.

##### 1.4.5.2 이해하기 쉬운 설계

변경이 가능한 코드란 이해하기 쉬운 코드다.
하나의 객체가 여러 객체에 의존성을 갖고 있어 이해하기 어렵다면 수정하기 어려울 것이다.

##### 1.4.5.3 적절한 의존성 관리

좋은 객체지향 설계란 협력하는 객체 사이의 의존성을 적절하게 관리하는 설계다.

> 세성에 엮인 것이 많은 사람일수록 변하기 어렵다.

객체가 싱행되는 주변 환경에 강하게 결합될수록 변경하기 어려워진다.